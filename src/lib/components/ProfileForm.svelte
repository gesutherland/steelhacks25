...
{:then p} 
    ...
    <div class="max-w-7xl mx-auto p-8 sm:p-12">
        <div>
        <h1 class="font-semibold text-4xl text-[#286480]">{p ? "Hello!" : "Hello Doctor"}</h1>
        <p class="font-light text-md text-slate-500 mt-1"> { p ? "Let's set up your patient profile" : "Edit your patients profile" }</p>
        </div>
...
=======
                    <input bind:value={item.name} type="text" id={"med-name-" + item.id} placeholder="Medication name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2">
>>>>>>> 3d4993f7dc532962249bcef889aed2fdbc1365cc
                    </div>
                    <div>
...
{#if !p}
  <div class="max-w-7xl mx-auto px-8 sm:px-12 mt-8">
    ...
    <div class="bg-slate-100 p-6 rounded-xl shadow-sm">
      <h2 class="text-xl font-bold text-slate-700 mb-4">Medical History</h2>
      <div>
        <label for="medical-history" class="sr-only">Medical History</label>
        <textarea 
